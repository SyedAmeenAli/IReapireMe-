...
import { ChevronRight, ChevronLeft, Search, Check, ArrowRight, Smartphone, Tablet, Laptop, Watch, Headphones, Monitor, Upload, FileText, CalendarCheck, Truck } from 'lucide-react';
...
export default function Repair() {
  ...
  const availableBrands = brands.filter((b) =>
    b.deviceTypes.some((dt) => dt === selectedDevice || (selectedDevice === 'android' && dt === 'android'))
  );

  const availableModels = deviceModels.filter(
    (m) => m.brand === selectedBrand && (m.type === selectedDevice || selectedDevice === 'android' && m.type === 'android')
  );
  ...
  const selectedModelData = deviceModels.find((m) => m.slug === selectedModel);
  const availableServicesList = selectedModelData
    ? services.filter((s) => selectedModelData.services.includes(s.slug))
    : services;

  const selectedService = services.find((s) => s.slug === selectedIssue);
  ...
  const handleBook = () => {
    ...
    updateBooking({
      ...
      issue: selectedIssue,
      priceLabel: selectedService ? `From ₹${selectedService.priceFrom.toLocaleString()}` : '',
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6 md:p-8">
                ...
                {Object.entries(deviceLabels).map(([key, label]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-b-sm font-medium text-neutral-950">{label}</span>
                    </button>
                  );
                })}
              </div>
                ...
                {availableBrands.map((brand) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all hover:border-neutral-400 ${
                      selectedBrand === brand.slug ? 'border-neutral-950 bg-neutral-50' : 'border-neutral-200'
                    }`}
                  >
                    <span className="text-b-sm font-medium text-neutral-950">{brand.name}</span>
                    <span className="text-b-xs text-neutral-500">{brand.popularModels.length} models</span>
                  </button>
                ))}
              </div>
            </div>
                ...
                {filteredModels.map((model) => (
                  <button
                    ...
                    className={`flex items-center justify-between p-3 rounded-lg border-2 transition-all hover:border-neutral-400 text-left ${
                      selectedModel === model.slug ? 'border-neutral-950 bg-neutral-50' : 'border-neutral-200'
                    }`}
                  >
                    <div>
                      <span className="text-b-sm font-medium text-neutral-950">{model.name}</span>
                      <span className="text-b-xs text-neutral-500 ml-2">{model.releaseYear}</span>
                    </div>
          ...
          {step === 'issue' && (
            <div>
              ...
              <p className="text-b-sm text-neutral-500 mb-6">{selectedModelData?.name}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {availableServicesList.map((service) => (
                  <button
                    key={service.id}
                    onClick={() => {
                      setSelectedIssue(service.slug);
                      setStep('quote');
                    }}
                    className={`flex flex-col items-start p-4 rounded-xl border-2 transition-all hover:border-neutral-400 text-left ${
                      selectedIssue === service.slug ? 'border-neutral-950 bg-neutral-50' : 'border-neutral-200'
                    }`}
                  >
                    ...
                    <span className="text-b-xs font-medium text-neutral-700 mt-1">From &#8377;{service.priceFrom.toLocaleString()}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 'quote' && selectedService && (
            <div>
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-b-sm text-neutral-500">{selectedModelData?.name} &middot; {selectedService.title}</p>
              </div>
              ...
              <div className="bg-neutral-50 rounded-xl p-5 border border-neutral-200 mb-6">
                <div className="text-center">
                  ...
                  <p className="text-b-xs text-neutral-500 mb-4">Starting price</p>
                </div>
